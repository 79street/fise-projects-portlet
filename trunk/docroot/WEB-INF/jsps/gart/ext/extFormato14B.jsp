...
var formato14B= {
		...
		formato14B.tablaResultados.jqGrid('navButtonAdd',formato14B.paginadoResultados,{
		       ...
		       onClickButton : function () {
			       ...
			       if(ids!=0){
			    	   ...
			    	   formato14B.dialogMessageInfoContent.html('No existe información para exportar a Excel');
	...
	inicializarFormulario : function(){
		...
		if(formato14B.i_codEmpresaB.val()!=''){
			formato14B.f_empresa.val(formato14B.i_codEmpresaB.val());
		}else{
	...
	soloNumerosEnteros : function(){
		formato14B.f_nroValesImpR.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesImpR',7,0)");
		formato14B.f_nroValesReptR.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesReptR',7,0)");
		formato14B.f_nroValesEntrR.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesEntrR',7,0)");
		formato14B.f_nroValesFisR.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesFisR',7,0)");
		formato14B.f_nroTotalAtenR.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroTotalAtenR',7,0)");
		
		formato14B.f_nroValesImpP.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesImpP',7,0)");
		formato14B.f_nroValesReptP.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesReptP',7,0)");
		formato14B.f_nroValesEntrP.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesEntrP',7,0)");
		formato14B.f_nroValesFisP.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesFisP',7,0)");
		formato14B.f_nroTotalAtenP.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroTotalAtenP',7,0)");
		
		formato14B.f_nroValesImpL.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesImpL',7,0)");
		formato14B.f_nroValesReptL.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesReptL',7,0)");
		formato14B.f_nroValesEntrL.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesEntrL',7,0)");
		formato14B.f_nroValesFisL.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroValesFisL',7,0)");
		formato14B.f_nroTotalAtenL.attr("onkeypress","return soloNumerosEntero(event, 1, 'nroTotalAtenL',7,0)");
	...
	editarFormato : function(codEmpresa,anoPresentacion,mesPresentacion,anoIniVigencia,anoFinVigencia,etapa,flagOperacion){	
		...
		if(flagOperacion=='ABIERTO'){
			...
			if(process){
				...
				formato14B.dialogMessageInfoContent.html('No tiene autorización para realizar esta acción');
			    formato14B.dialogMessageInfo.dialog("open");
			}	
			
		}else if(flagOperacion=='CERRADO'){
			//alert(" Está fuera de plazo");
			formato14B.dialogMessageInfoContent.html('El plazo para realizar esta acción se encuentra cerrado');
		    formato14B.dialogMessageInfo.dialog("open");
		}else{
...
//////CRUD
			 ...
			 jQuery.ajax({
				...
				success: function(data) {			
					if (data.resultado == "OK1"){				
						...
						var addhtml2='El Formato 14B se actualizó satisfactoriamente';
						formato14B.dialogMessageContent.html(addhtml2);
						formato14B.dialogMessage.dialog("open");
						formato14B.flagCarga.val('1');//inicializamos el flag de carga cuando editamos el archivo antes de cargar archivos
						formato14B.mostrarFormularioModificado();
	...
	eliminarFormato : function(cod_Empresa,ano_Presentacion,mes_Presentacion,ano_Inicio_Vigencia,ano_Fin_Vigencia,cod_Etapa){			
		...
		jQuery.ajax({
			...
			success: function(data) {
				if (data.resultado == "OK"){
					var addhtml2='El registro seleccionado para el Formato 14B se eliminó satisfactoriamente';					
					formato14B.dialogMessageContent.html(addhtml2);
					formato14B.dialogMessage.dialog("open");
					formato14B.buscar();
					formato14B.initBlockUI();
	...
	<portlet:namespace/>mostrarFormularioCargaExcel : function(){
		if (formato14B.validarArchivoCarga()){
		    ...
		    formato14B.dialogCargaExcel.draggable();
	...
	<portlet:namespace/>mostrarFormularioCargaTexto : function(){
		if (formato14B.validarArchivoCarga()){
			...
			formato14B.dialogCargaTexto.draggable();
	...
	confirmarEnvioDefinitivo : function(){	
		var addhtml='¿Está seguro que desea realizar el Envío Definitivo para el Formato 14B?';
	...
	buildGridsObservacion : function () {	
		...
		formato14B.tablaObservacion.jqGrid('navButtonAdd',formato14B.paginadoObservacion,{
		       ...
		       buttonicon: "ui-icon-bookmark",
		       onClickButton : function () {
		    	   var ids = formato14B.tablaObservacion.jqGrid('getDataIDs');
			       if(ids!=0){
			    	   location.href = '<%=renderResponse.encodeURL(renderRequest.getContextPath()+"/ExportExcelPlus")%>';   
			       }else{			    	
			    	   formato14B.dialogMessageInfoContent.html('No existe información para exportar a Excel');
		...
		formato14B.tablaObservacion.jqGrid('navButtonAdd',formato14B.paginadoObservacion,{
		       ...
		       onClickButton : function () {
			       ...
			       if(ids!=0){
			    	   ...
			    	   formato14B.dialogMessageInfoContent.html('No existe información para exportar a Pdf');
			    	   formato14B.dialogMessageInfo.dialog("open");
			       }     	   
		       } 
		});
	...
	<portlet:namespace/>mostrarReporteActaEnvio : function(){
		...
		if(estado=='Enviado'){
			...
			formato14B.dialogMessageInfoContent.html('Primero debe realizar el Envío Definitivo del Formato 14B');
			formato14B.dialogMessageInfo.dialog("open");
		}
	},
