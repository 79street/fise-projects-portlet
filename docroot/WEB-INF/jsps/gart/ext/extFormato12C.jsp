...
var formato12C= {
		...
		formato12C.tablaResultados.jqGrid('navButtonAdd',formato12C.paginadoResultados,{
		       ...
		       onClickButton : function () {
			       ...
			       if(ids!=0){
			    	...
			    	formato12C.dialogMessageInfoContent.html('No existe información para exportar a Excel');
		...
		formato12C.tablaImplementacion.jqGrid('navButtonAdd',formato12C.paginadoImplementacion,{
		       ...
		       onClickButton : function () {
			       ...
			       if(ids!=0){
			    	...
			    	formato12C.dialogMessageInfoCrudContent.html('No existe información para exportar a Excel');
	...
	validarBusqueda : function(){
		if(formato12C.anioDesde.val().length != '' ) {		  
			  ...
			  formato12C.dialogMessageWarningContent.html('Debe ingresar un Año Declarado Desde');
			  formato12C.dialogMessageWarning.dialog("open");
			  formato12C.anioDesde.focus();
			  return false;
		  }
		  if(formato12C.anioHasta.val().length != '' ) {		  
			  ...
			  formato12C.dialogMessageWarningContent.html('Debe ingresar un Año Declarado Hasta');
			  formato12C.dialogMessageWarning.dialog("open");
			  formato12C.anioHasta.focus();
			  return false;
		  }
	...
	validarGrupoInformacion : function(){
		if( $('#idGrupoInfo').val()=='0' ){
			...
			formato12C.dialogMessageWarningCrudContent.html('Primero debe estar creado el Grupo de Información Mensual para el Año y Mes a declarar');
		...
		jQuery.ajax({
			...
			success : function(data) {
				if(data.resultado == "OK"){
					...
					var addhtmObs = 'No se puede relizar el Envío Definitivo del Formato 12C, primero debe subsanar las observaciones.';				
					formato12C.dialogMessageInfoCrudContent.html(addhtmObs);
					formato12C.dialogMessageInfoCrud.dialog("open");
					formato12C.unblockUI();					
				}else{								
					var addhtmError='Error al realizar el Envío Definitivo del Formato 12C.';					
		...
		formato12C.tablaObservacion.jqGrid('navButtonAdd',formato12C.paginadoObservacion,{
		       ...
		       onClickButton : function () {
			       ...
			       if(ids!=0){
			    	...
			    	formato12C.dialogMessageInfoCrudContent.html('No existe información para exportar a Pdf');
					formato12C.dialogMessageInfoCrud.dialog("open");
			       }   	   
		       } 
		});
	...
	validarFormatoDetalle : function(){
		...
		if(formato12C.mesEjecucionDetalle.val().length == '' ) {		  
		   ...
		   formato12C.dialogMessageWarningDetalleContent.html('Debe ingresar el Mes de Ejecución');
			formato12C.dialogMessageWarningDetalle.dialog("open");
		...
		if(formato12C.etapaEjecucionDetalle.val().length == '' ) {		  
		   //alert('Debe ingresar la etapa de ejecución');
		   formato12C.dialogMessageWarningDetalleContent.html('Debe ingresar la Etapa de Ejecución');
			formato12C.dialogMessageWarningDetalle.dialog("open");
		   ...
		   return false; 
			...
			if(formato12C.cod_empresa_edelnor.val()!=formato12C.codEmpresaDetalle.val() && formato12C.cod_empresa_luz_sur.val()!=formato12C.codEmpresaDetalle.val()){
				if(formato12C.idZonaBenefDetalle.val() == 3 ) {//RURAL
					...
					formato12C.dialogMessageWarningDetalleContent.html('No puede seleccionar la Zona Beneficiario Lima para la Distribuidora Eléctrica');
					formato12C.dialogMessageWarningDetalle.dialog("open");
					formato12C.idZonaBenefDetalle.focus();
				   	return false;
				}
