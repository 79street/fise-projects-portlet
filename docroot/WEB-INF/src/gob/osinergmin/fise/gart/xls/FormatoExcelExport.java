...
public class FormatoExcelExport {
	...
	public HSSFSheet construirExcelFormato14C(List<?> listaFormato14C) {
		...
		if( listaFormato14C!=null && listaFormato14C.size()>0 ){
			for( int i=0;i<listaFormato14C.size();i++ ){
				...
				//ax7.setCellValue(new HSSFRichTextString((fiseFormato14C.getFechaEnvioDefinitivo()==null)?FiseConstants.ESTADO_POR_ENVIAR_F14C:FiseConstants.ESTADO_ENVIADO_F14C));
				ax7.setCellStyle(dateCellStyle);
			}
		}
	...
	public HSSFSheet construirExcelMantenimientoTipoDocReferencia(List<?> listaDocRef) {
		...
		HSSFCell a1 = segundaFila.createCell(1);
		a1.setCellValue(new HSSFRichTextString("IdTipoDocReferencia"));
		a1.setCellStyle(headerCellStyle);
		...
		if(listaDocRef!=null && listaDocRef.size()>0 ){
			for( int i=0;i<listaDocRef.size();i++ ){
				
				FiseTipDocRef obs = (FiseTipDocRef) listaDocRef.get(i);
				System.out.println("Entrando a llenar filas y celdas en el exel tipo doc referencia"); 
				HSSFRow fila = _xlsSheet.createRow(i+1);
				
				HSSFCell ax1 = fila.createCell(1);
				ax1.setCellValue(new HSSFRichTextString(obs.getIdTipDocRef()));
				ax1.setCellStyle(dateCellStyle);
				HSSFCell ax2 = fila.createCell(2);
				ax2.setCellValue(new HSSFRichTextString(obs.getDescripcion()));
				ax2.setCellStyle(dateCellStyle);			
	...
	public HSSFSheet construirExcelMantenimientoParametros(List<?> listaParam) {
		...
		HSSFCell a1 = segundaFila.createCell(1);
		a1.setCellValue(new HSSFRichTextString("Código"));
		a1.setCellStyle(headerCellStyle);
		
		HSSFCell a2 = segundaFila.createCell(2);
		a2.setCellValue(new HSSFRichTextString("Nombre"));
		a2.setCellStyle(headerCellStyle);
		
		HSSFCell a3 = segundaFila.createCell(3);
		a3.setCellValue(new HSSFRichTextString("Valor"));
		a3.setCellStyle(headerCellStyle);
		
		HSSFCell a4 = segundaFila.createCell(4);
		a4.setCellValue(new HSSFRichTextString("Orden"));
		a4.setCellStyle(headerCellStyle);

		if( listaParam!=null && listaParam.size()>0 ){
			for( int i=0;i<listaParam.size();i++ ){
				
				FiseParametro param = (FiseParametro) listaParam.get(i);
				System.out.println("Entrando a llenar filas y celdas en el excel parametros"); 
				HSSFRow fila = _xlsSheet.createRow(i+1);
				
				HSSFCell ax1 = fila.createCell(1);
				ax1.setCellValue(new HSSFRichTextString(""+param.getCodigo()));
				ax1.setCellStyle(dateCellStyle);
				HSSFCell ax2 = fila.createCell(2);
				ax2.setCellValue(new HSSFRichTextString(""+param.getNombre()));
				ax2.setCellStyle(dateCellStyle);	
				HSSFCell ax3 = fila.createCell(3);
				ax3.setCellValue(new HSSFRichTextString(""+param.getValor()));
				ax3.setCellStyle(dateCellStyle);
				HSSFCell ax4 = fila.createCell(4);
				ax4.setCellValue(new HSSFRichTextString(""+param.getOrden()));
				ax4.setCellStyle(dateCellStyle);
			}
		}
		/**
	...
	//
		...
		public HSSFSheet construirExcelVariacion(List<?> listaVariacion) {
			...
			if( listaVariacion!=null && listaVariacion.size()>0 ){
				for( int i=0;i<listaVariacion.size();i++ ){
					...
					ax1.setCellStyle(dateCellStyle);
					HSSFCell ax2 = fila.createCell(2);
					ax2.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
					ax2.setCellValue(Double.parseDouble(var.getValor()));//anio Pres
					ax2.setCellStyle(dateCellStyle);
		...
		//historico de costos
				public HSSFSheet construirExcelHistorico(List<?> listaHistorico) {
					...
					if( listaHistorico!=null && listaHistorico.size()>0 ){
						for( int i=0;i<listaHistorico.size();i++ ){
							...
							ax2.setCellType(HSSFCell.CELL_TYPE_NUMERIC);
							ax2.setCellValue(Double.parseDouble(cociente));
							ax2.setCellStyle(dateCellStyle);
	...
	public HSSFSheet FillExcelSheet(XlsWorksheetConfig xlsWorksheetConfig, HSSFSheet xlsSheet, HSSFWorkbook wb) {
		...
		for (XlsTableConfig xlsTableConfig : xlsWorksheetConfig.getTables()) {
			...
			}else if( FiseConstants.PARAMETROS_EXPORT_EXCEL.equals(xlsTableConfig.getTipoFormato()) ){
				construirExcelMantenimientoParametros(xlsTableConfig.getLista());				
			...
			else if( FiseConstants.TIPO_FORMATO_HISTORICO.equals(xlsTableConfig.getTipoFormato())){
				construirExcelHistorico(xlsTableConfig.getLista());
			}
		}
		return _xlsSheet;
	}
